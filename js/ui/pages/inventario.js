export function renderInventoryPage(container, cultivation, helpers) {
  const { card, escape } = helpers;
  const items = cultivation.inventario
    .map(
      (item) =>
        `<div class="list-row"><div><b>${escape(item.nombre)}</b><br><small>Umbral: ${item.umbralBajo ?? "—"} ${escape(item.unidad)}</small></div><b>${item.cantidad} ${escape(item.unidad)}</b></div>`,
    )
    .join("");
